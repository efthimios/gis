...
/*********
  New DD  
 *********/
if (isset($_GET["new_dd"])) { // called by dd_ops.js
  $dd = $_GET["new_dd"];
  
  $query = "SELECT * FROM xDim_Diamerisma WHERE onoma='". in($dd) . "'";
  ...
  if ($num_rows > 0) {
    echo "Το όνομα του ΔΔ που εισάγατε είναι ήδη καταχωρημένο";
  }
  else {
?>
<form method="POST" onsubmit="DD_D_N.php">
	<input type=hidden name="new_dd" >
	<table>
		<tr><td><input type=hidden name="onoma" value=<? echo $dd; ?>></td></tr>
		<tr><td>ONOMA: <? echo $dd; ?></td></tr>
    <tr><td>ΔΗΜΟΣ:</td><td><? dropdown("xDimos","onoma", "", "");  ?></td></tr>
		<tr><td>ΕΜΒΑΔΟ:</td><td><input type=text size="20" name="emv" /></td></tr>
		<tr><td>ΟΗΠ:</td><td><input type=text size="20" name="ohp" /></td></tr>
		<tr><td>ΠΛΗΘΥΣΜΟΣ:</td><td><input type=text size="20" name="plith" /></td></tr>
		<tr><td>ΣΥΝΤΕΛΕΣΤΗΣ:</td><td><input type=text size="20" name="sint" /></td></tr>
		<tr><td>ΑΡ. ΚΛΙΝΩΝ:</td><td><input type=text size="20" name="klines" /></td></tr>
		<tr><td>ADV:</td><td><input type=text size="20" name="adv" /></td></tr>
		<tr><td>ΕΕΛ:</td><td>
		  
    <?
      // Find EEL eidos_pigis ID
      $query = "SELECT id FROM xEidos_pigis_ripansis WHERE onoma='". in("ΕΕΛ") . "'";
      $rs = odbc_exec($conn,$query);
      if (!$rs) {exit("Error in SQL");}
      
			$eidos = " ";
      while (odbc_fetch_row($rs)) {
        $eidos = out(odbc_result($rs,"id"));
      }
			dropdown("xPigi_ripansis", "onoma", "eidos_pigis_id=$eidos", "");	  
	?>
	</td></tr>
